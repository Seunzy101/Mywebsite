  ...
  const fetchBookings = async () => {
    try {
      const response = await axios.get(
        "http://localhost/tulip-backend/api/bookings/get.php?user_id=1"
      );
        ...
        {filteredBookings.length === 0 ? (
          ...
          <div className="flex flex-col gap-6">
            ...
            {filteredBookings.map((booking) => (
              <div
                key={booking.id}
                className="bg-white rounded-3xl shadow-md p-6 flex flex-col md:flex-row md:justify-between gap-6"
              >
                <div>
